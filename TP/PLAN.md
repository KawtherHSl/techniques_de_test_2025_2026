...
1) Tests unitaires:
   ...
   pourquoi:ce teste garantie que le module parsing fonctionne correctement et prend bien en charge les erreurs des formats sans bloquer le focntionnement du code.
   
   1.2.  Tetser l'ncodage de triangle: la focntion prend en entré une liste des triangles par exemple : [(1,2,3)] càd nous avons un seul triangle avec les points 1 2 3, et esaaie de convertir ce triangle en format binaire.
   comment: on écrit un test automatique qui prend la liste des trinagle en entrée et compare avec le flux binaire attendue, si ce n'est pas conforme elle déclenche une exception.
...
3) Tests de l'api triangulator (http):
   comment:
   pourquoi:vérifier la conformité du service REST.

4) Test de performance: c'est pour mesurer le temps de décodage et de triangulation.
   comment: on teste tous d'abord avec 10 points puis 100 points et in fini ce teste par les 1000 points et on mesure le temps avec une fonction de timing.
...
6) Test de qualité: dans ce ca son utilise setPoint vide ou pointSetManager non disponible.
